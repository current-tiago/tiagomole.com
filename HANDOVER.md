...
1. **Hero** — redesigned June 2026 into a warm, hand-drawn "scrapbook" (was a clinical two-column editorial split). Centred handwritten title "Rocky's *Home*" (Caveat font) with a hand-drawn squiggle underline; "made just for you, Imy" eyebrow; soppy subtitle; two **stick-figure doodles holding hands** (orange Tiago + pink Imy + a heart); floating SVG doodles (sun, cloud, hearts, **Rocky the crab**); nav rendered as little wonky paper tabs. Notebook-ruled paper background + pink/orange radial blush. Fonts added: **Caveat** (`--hand`) + **Gloria Hallelujah** (`--doodle`). All doodles are inline SVG (`.doodle`, stroke `currentColor`, `vector-effect:non-scaling-stroke`).
   - **Countdown** — a pinned paper "ticket" (`#countdown-wrap`, top-right desktop / inline on mobile): label set by JS to "Until I see you next" + DD:HH:MM:SS down to **June 24 2026, 23:00 Lisbon** (`2026-06-24T23:00:00+01:00`); hides itself once reached. Update `nextVisit` for future visits.
   - **Meta count-up** (`#since-clock`): now a yellow taped sticky-note (bottom-left desktop; hidden on mobile). Calendar-aware "Xy Xm Xd Xh" ticking up from **April 11 2026, 01:00 Lisbon** (`2026-04-11T01:00:00+01:00`).
   - ⚠️ JS hooks that must be preserved on any hero edit: `#countdown-wrap`, `.hero-countdown-label`, `#countdown-time`, `#countdown-sub`, `#since-clock`.
2. **Seven Poems** (`#poems`, labelled "Seven Little Love Letters") — redesigned June 2026 from an accordion into a 3-col grid of **openable love-letter envelopes** (kraft envelope + lifting flap + wax heart seal "T♥I" + roman-numeral postage stamp). Click/Enter toggles `.open` on the `.envelope` (`toggleEnvelope` / `envelopeKey` in JS) — flap lifts, seal fades, and the poem unfolds below on lined notebook paper with a handwritten sign-off. Each opens independently (no longer one-at-a-time). Verse text unchanged; `.poem-stanzas` / `.stanza p` classes retained.
   - I · Sample no.1 · II · View of the Room · III · T-3 Weeks · IV · Studying · V · Orpheus · VI · Summer Afternoon · VII · Three Months
   - **VII "Three Months"** is the 3-month-anniversary letter (prose, not verse). It carries class `env-locked` and stays hidden until the anniversary has passed — revealed only when `document.body` has `anniversary-passed` (Lisbon date ≥ 2026-07-12).
   - **Anniversary takeover:** for the whole of **11 July 2026 (Lisbon)** the page shows *only* this letter — `#anniversary-letter`, a fixed full-screen lined-paper card. `checkLetterDay()` (runs on load + every 60 s) sets `body.letter-day` on that date, and `body.letter-day > *:not(#anniversary-letter)` hides everything else; it reverts automatically at midnight, after which the letter lives on as envelope VII. Both copies of the letter text are verbatim — do not paraphrase.
...
5. **Thoughts About You** (`#thoughts`) — Tiago's daily thoughts, posted from his phone, revealed at **21:00 Lisbon** each day
   ...
   - **Admin ops** (same token): `{token, op:'undo'}` removes the latest entry; `{token, op:'clear'}` wipes all
   - ⚠️ Netlify **secrets scanning** failed builds because the read key is deliberately embedded in the gated page — `netlify.toml` sets `SECRETS_SCAN_OMIT_KEYS = "THOUGHTS_READ_KEY"`. Never add the write token to that omit list.
   - **GET** `?k=<THOUGHTS_READ_KEY>` returns **tonight's batch, and only after 21:00 Lisbon** (`REVEAL_HOUR`). A batch = every thought written in the rolling window from the previous 21:00 up to tonight's 21:00 (i.e. yesterday-9pm → today-9pm), keyed on each thought's `ts` via `revealDateOf()`. A thought written after 9pm rolls into the next night's batch. Older batches stay stored but are never returned. Page re-fetches every 5 min + on tab focus, so the reveal appears without a reload
